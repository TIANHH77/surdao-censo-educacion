...
if "Gemini 1.5 Pro" in modelo_elegido:
    llm = ChatGoogleGenerativeAI(model="gemini-1.5-pro", temperature=0) # Actualizado al modelo estable
elif "Gemini 1.5 Flash" in modelo_elegido:
    llm = ChatGoogleGenerativeAI(model="gemini-1.5-flash", temperature=0) # Actualizado al modelo estable
...
def cargar_datos():
    ...
    # Manejo seguro de carga: Si no existe un archivo, se ignora o se muestra advertencia.
    dfs = {}
    archivos_esperados = {
    ...
    for nombre, ruta in archivos_esperados.items():
        if os.path.exists(ruta):
            ...
            st.sidebar.warning(f"Archivo no encontrado: {nombre}")
...
archivos_seleccionados = st.sidebar.multiselect(
    ...
    default=nombres_archivos[:2] if nombres_archivos else [],
...
if "Matriz_Educacion" in archivos_seleccionados and "Matriz_Educacion" in diccionario_dfs:
    ...
    if "LATITUD" in df_mapa.columns and "LONGITUD" in df_mapa.columns:
        ...
        df_mapa_limpio = df_mapa.copy().fillna("Sin registro")
        ...
        st.pydeck_chart(
            pdk.Deck(
                ...
                tooltip={
                    ...
                    "📚 Promedio: <b>{Promedio_Notas_Anio}</b><br/>"
                    "📉 Volatilidad: <b>{Volatilidad_Rendimiento}</b><br/>"
                    "⚖️ Ratio Alumnos/Profe: <b>{Ratio_Alumnos_Docente}</b><br/>"
...
prompt = ChatPromptTemplate.from_messages([
    ...
    MessagesPlaceholder(variable_name="chat_history"), # Integración nativa de memoria
    ("human", "{input}"),
    MessagesPlaceholder(variable_name="agent_scratchpad"),
])

agente_base = create_tool_calling_agent(llm=llm, tools=[ejecutar_pandas], prompt=prompt)
...
if pregunta:
    # 1. Mostrar la pregunta visualmente
    st.session_state.mensajes.append({"role": "user", "content": pregunta})
    st.chat_message("user").write(pregunta)

    # 2. Reconstruir el historial en objetos nativos de LangChain
    chat_history = []
    # Usamos todo el historial, excluyendo el mensaje actual (que se envía en 'input')
    ...
    with st.spinner("🕵️‍♂️ El Auditor está analizando los datos..."):
        try:
            respuesta = agente.invoke({
                "input": pregunta, 
                "chat_history": chat_history # Pasamos la lista de objetos aquí
            })
            
            # Limpiamos el prefijo si el modelo lo generó
