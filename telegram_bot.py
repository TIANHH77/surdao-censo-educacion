...
- Mensaje de bienvenida adaptado a 11 bases maestras
- Token seguro por variable de entorno
- Caché de respuestas asíncrona ligada a Chat ID
...
# ──────────────────────────────────────────────
# CARGA DEL DATAMART (Con soporte para subcarpetas)
# ──────────────────────────────────────────────
def cargar_datamart_completo():
    ...
    # os.walk recorre la raíz y TODAS las subcarpetas automáticamente
    ...
    print(f"\n🔥 DATAMART LISTO: {len(dfs)} bases maestras activas en memoria RAM.")
    return dfs
...
# ──────────────────────────────────────────────
# ESTADO GLOBAL & CACHÉ CON TTL LIGADA AL CHAT ID
# ──────────────────────────────────────────────
historiales: dict[int, list] = {}
cache_respuestas: dict[str, tuple[str, float]] = {}

def hash_pregunta(chat_id: int, pregunta: str) -> str:
    return hashlib.md5(f"{chat_id}_{pregunta.lower().strip()}".encode()).hexdigest()

def obtener_cache(chat_id: int, pregunta: str) -> str | None:
    h = hash_pregunta(chat_id, pregunta)
...
def guardar_cache(chat_id: int, pregunta: str, respuesta: str):
    h = hash_pregunta(chat_id, pregunta)
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    mensaje_bienvenida = (
        ...
        "Tengo en mi memoria un datamart avanzado con **11 bases maestras consolidadas** (Censo 2024 y MINEDUC).\n\n"
        ...
        "También puedes usar `/buscar [tema]` para encontrar bases en mi memoria.\n\n"
...
async def responder(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        output_final = obtener_cache(chat_id, pregunta)
        
        if not output_final:
            # Ejecución asíncrona para no bloquear el bot
            respuesta = await asyncio.to_thread(invocar_con_fallback, agente, {"input": pregunta, "chat_history": chat_history})
            ...
            if "stopped due to max iterations" in output_final.lower() or "agent stopped" in output_final.lower():
                ...
                guardar_cache(chat_id, pregunta, output_final)
...
async def ayuda(update: Update, context: ContextTypes.DEFAULT_TYPE):
    msg = (
        ...
        "/buscar [tema] — Busca bases por tema\n"
